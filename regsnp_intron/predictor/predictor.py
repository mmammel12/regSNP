...
class Predictor(object):
    ...
    def predict(self, ifname, ofname):
        ...
        result = pd.concat([data_on_ss, data_off_ss]).sort_values(by=["#chrom", "pos"])
    ...
    def _predict_on_ss(self, data):
        ...
        """
        data = data.drop(
            ["#chrom", "pos", "ref", "alt", "name", "strand", "distance"], axis=1
        )
...
def main():
    ...
    parser.add_argument("db_path", help="path to models")
    parser.add_argument(
        "sfname", help="SNV file with features generated by feature calculator"
    )
    parser.add_argument("ofname", help="output file prefix")
    args = parser.parse_args()
    predictor = Predictor(
        os.path.join(args.db_path, "on_ss_imp.pkl"),
