...
class FeatureCalculator(object):
    ...
    def _merge_features(self):
        ...
        out_dir_tmp = os.path.join(self.out_dir, 'tmp')
        rbp_change = pd.read_csv(os.path.join(
            out_dir_tmp, 'snp.rbp_change'), sep='\t', header=0)
        snps = rbp_change.loc[:, ['#chrom', 'pos', 'ref', 'alt']]
        rbp_change.drop(['#chrom', 'pos', 'ref', 'alt',
                         'ref_seq', 'alt_seq'], axis=1, inplace=True)

        column_to_drop = ['#chrom_snp', 'start_snp', 'end_snp', 'ref', 'alt', 'feature',
                          'gene_id', 'chrom', 'start', 'end', 'score']
        protein_feature = pd.read_csv(os.path.join(
            out_dir_tmp, 'snp.protein_feature'), sep='\t', header=0)
        ...
        result = pd.concat([snps, rbp_change, protein_feature,
                            junction_strength, phylop], axis=1)
